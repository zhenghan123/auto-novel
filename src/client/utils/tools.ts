...
export function epubMaker(
    ...
    return new Promise((resolve, reject) => {
        ...
        exec(pandocCommand, (error, stdout, stderr) => {
            ...
            console.log(pandocCommand.split("&&"));
            resolve();
        });
    });
}
...
export async function questionAccount() {
    const userName = await question(colorize("输入账号：", "purple"));
    const passWord = await question(colorize("输入密码：", "purple"));
...
// 表格输出，稍作美化
export async function selectBookFromList(books: any[]): Promise<number> {
    ...
    const index = await question(
        `请输入${colorize("[1]", "blue")}~${colorize(`[${books.length}]`, "blue")}序号：`
    );
