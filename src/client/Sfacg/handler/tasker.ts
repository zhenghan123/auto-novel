...
export class _SfacgTasker {
    ...
    async checkAndClaimRewards() {
        ...
        PendingRewards &&
            PendingRewards.map(async (task: tasks) => {
                task.status == 1 &&
                    task.name !== "每日签到" &&
                    (await this.Client.taskBonus(task.taskId));
            });
        const logTask = await this.Client.getTasks();
        logTask &&
            logTask.map((task: tasks) => {
                task.name !== "每日签到" &&
                    (task.status === 2
                        ? this.success.push(task.name + "成功")
                        : this.failed.push(task.name + "失败"));
            });
    }
