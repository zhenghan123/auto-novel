...
export class _SfacgCache {
  ...
  static async UpsertChapterInfo(chapters: _dbChapters) {
    ...
    console.log(
      `UpsertChapterInfo successfully ${colorize(
        `${chapters.chapId}`,
        "green"
      )}`
    );
  ...
  private static async UpsertAccount(accountInfo: IaccountInfo) {
    ...
    console.log(
      `UpsertAccount successfully ${colorize(
        `${accountInfo.userName}`,
        "green"
      )}`
    );
  ...
  static async RemoveAccount(userName: string) {
    ...
    if (error) {
      console.log(
        `Error removeAccount: ${colorize(`${userName}`, "purple")} `,
        error
      );
      return null;
    }
    console.log(
      `removeAccount successfully ${colorize(`${userName}`, "green")}`
    );
  }
  ...
  static async GetNovelList() {
    const { data, error } = await Server.from("Sfacg-novelInfos")
      .select("*")
      .order("novelId");
  ...
  static async GetChapterIdsByField(
    ...
    while (true) {
      ...
      if (error) {
        console.error(
          `Error GetChapterIdsByField: ${fieldName}=${fieldValue}`,
          error
        );
        return null;
      }
  ...
  static async GetChapterContent(chapId: number) {
    ...
    if (error) {
      console.log(
        `Error GetChapterContent: ${colorize(`${chapId}`, "purple")} `,
        error
      );
      return null;
    }
